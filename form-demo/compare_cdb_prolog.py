...
for FormLib (form variable comparison).

Runs both:
1. CDB attached to Python loading FormLib.dll (compiled Clarion)
2. SWI-Prolog interpreter executing FormDemo.clw source

Extracts procedure ENTER/EXIT events with arguments and return values,
then compares them line by line — including internal variable values
accessed via FLGetVar.
...
# Variable ID -> name mapping for readable output
VAR_NAMES = {1: "SensorID", 2: "Reading", 3: "Weight", 4: "Result", 5: "SensorType"}
...
def run_cdb_trace():
    ...
    # Parse TRACE_ENTER / TRACE_EXIT pairs
    lines = output.split("\n")
    trace = []
    i = 0
    while i < len(lines):
        ...
        if line.startswith("TRACE_ENTER "):
            ...
            while i < len(lines):
                l = lines[i].strip()
                if l.startswith("arg"):
                    # Next line has the hex value from dd command
                    i += 1
                    if i < len(lines):
...
def annotate_line(line):
    """Add human-readable annotation for FLGetVar calls."""
    m = re.match(r'CALL FLGetVar\((\d+)\) -> (-?\d+)', line)
    if m:
...
def log_and_print(line, log_lines):
    """Print to stdout and collect for log file."""
    print(line)
...
def main():
    ...
    log_and_print("=" * 65, log_lines)
    log_and_print("FormLib: CDB Debugger vs Prolog Interpreter Variable Comparison", log_lines)
    log_and_print(f"Run: {timestamp}", log_lines)
    log_and_print("=" * 65, log_lines)

    log_and_print("\n--- Running CDB trace (compiled FormLib.dll) ---", log_lines)
    ...
    if all_match and len(cdb_trace) == len(prolog_trace) and len(cdb_trace) > 0:
        ...
        log_and_print(f"  (including {sum(1 for l in cdb_trace if 'FLGetVar' in l)} variable value checks)", log_lines)
        result = 0
    else:
