  ...
  MAP
    OFRunForm(),LONG,C,NAME('OFRunForm'),EXPORT
    OFDoCalc(),LONG,C,NAME('OFDoCalc'),EXPORT
    OFDoClear(),LONG,C,NAME('OFDoClear'),EXPORT
    OFGetVar(LONG id),LONG,C,NAME('OFGetVar'),EXPORT
    ISqrt(LONG),LONG
  END
...
! OFRunForm -- open window, run ACCEPT loop, return magnitude on close
OFRunForm PROCEDURE()
  CODE
  APValue = 0
  APDir = 1
...
! OFDoCalc -- normalize negatives, flip directions, compute magnitude
OFDoCalc PROCEDURE()
  CODE
  IF APValue < 0
    APValue = 0 - APValue
...
! OFDoClear -- reset all fields and directions
OFDoClear PROCEDURE()
  CODE
  APValue = 0
  SIValue = 0
...
! OFGetVar -- read any variable by ID
OFGetVar PROCEDURE(LONG id)
  CODE
  CASE id
  OF 1
    RETURN APValue
...
! Integer square root via Newtons method
