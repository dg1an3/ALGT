%============================================================
% interpreter_logic.pl - Core of the Clarion AST Execution Engine
%
...
init_globals([queue(Name, Fields)|Rest], StateIn, StateOut) :-
    % Treat queue as an in-memory file
    create_empty_buffer(Fields, Buffer),
    FileState = file_state(Name, '', [], Fields, [], Buffer, -1, true), % Queues are always "open"
...
exec_statement(member_assign(VarName, FieldName, Expr), StateIn, StateOut, normal) :-
    ...
    ; % Existing logic for GROUP and instance
        eval_expr(Expr, StateIn, Value),
