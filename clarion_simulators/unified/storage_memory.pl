%============================================================
% storage_memory.pl - In-Memory Storage Backend
% Uses 8-element tuple: file_state(Name, Prefix, Keys, Fields, Records, Buffer, Position, IsOpen)
%============================================================
...
mem_add(file_state(N,Pre,K,F,Records,Buffer,_,O), file_state(N,Pre,K,F,NewRecords,Buffer,-1,O)) :-
    append(Records, [Buffer], NewRecords).

mem_get(key_search(KeyName, KeyFields, SearchValues), 
...
mem_put(file_state(N,Pre,K,F,Records,Buffer,Pos,O), file_state(N,Pre,K,F,NewRecords,Buffer,Pos,O)) :-
    Pos >= 0, length(Records, NumRecords), Pos < NumRecords,
    replace_nth0(Pos, Records, Buffer, NewRecords).

mem_delete(file_state(N,Pre,K,F,Records,B,Pos,O), file_state(N,Pre,K,F,NewRecords,B,-1,O)) :-
    Pos >= 0, delete_nth0(Pos, Records, NewRecords).

mem_next(file_state(N,Pre,K,F,Records,_,Pos,O), file_state(N,Pre,K,F,Records,NewBuffer,NextPos,O)) :-
    NextPos is Pos + 1, length(Records, NumRecords),
    NextPos < NumRecords, nth0(NextPos, Records, NewBuffer).

mem_set(file_state(N,Pre,K,F,R,B,_,O), file_state(N,Pre,K,F,R,B,-1,O)).
mem_records(file_state(_,_,_,_,Records,_,_,_), Count) :- length(Records, Count).
mem_empty(file_state(N,Pre,K,F,_,B,_,O), file_state(N,Pre,K,F,[],B,-1,O)).
mem_clear(file_state(N,Pre,K,Fields,R,_,P,O), file_state(N,Pre,K,Fields,R,NewBuffer,P,O)) :-
    create_default_buffer(Fields, NewBuffer).

create_default_buffer([], []).
create_default_buffer([field(_, Type, Size)|Rest], [Value|Values]) :-
    default_value(Type, Size, Value), create_default_buffer(Rest, Values).
...
replace_nth0(N, [H|T], Elem, [H|R]) :- N > 0, N1 is N - 1, replace_nth0(N1, T, Elem, R).
